...
filename = "sample"
num_class = 3
num_sample = 300  # 每一个类别的样本数
len_sample = 10240  # 每一个样本的长度
x = np.zeros((1, len_sample))
for i in range(num_class):
    dataFile = directionary + '/' + filename + str(i + 1) + '.mat'
    data = np.transpose(io.loadmat(dataFile)['data'])
    x = np.vstack((x, data))
x = np.delete(x, 0, 0)
y = np.zeros((1, 1))
for i in range(num_class):
...
# history value:
m_c = 11.96808511e-2 * torch.ones(1, row * col)
m_n = 11.96808511e-2 * torch.ones(1, row * col)
...
k_n = 0.0e-3 + 2.0e-3 * torch.rand(1, row * col, dtype=torch.float32) / beta**2

# k_c = 5e-3 * torch.ones(1, (row - 1) * col + row * (col - 1)) / beta**2
k_c = 0.1e-3 + 0.9e-3 * torch.rand(1, (row - 1) * col + row * (col - 1), dtype=torch.float32) / beta**2
...
for epoch in range(1, max_epoch+1):
    ...
    net = torch.load("./model/" + model_filename)
